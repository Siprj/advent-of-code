...
fn part_1(input: &str, pair_count: usize) -> String {
    let boxes = parse(input);
    let pairs = get_shortest_distances(&boxes, pair_count);
    ...
    let mut sizes: Vec<usize> = circuit_to_points
        .values()
        .map(|points| points.len())
        .collect();
    sizes.sort();
    sizes.iter().rev().take(3).product::<usize>().to_string()
}

fn get_shortest_distances(points: &[Point], pair_count: usize) -> Vec<(isize, Point, Point)> {
    let mut res: BTreeSet<(isize, Point, Point)> = BTreeSet::new();

    for (index, p1) in points.iter().enumerate() {
        let from = index + 1;
        for p2 in points[from..].iter() {
            let distance = p1.distance_squared(p2);
            res.insert((distance, p1.clone(), p2.clone()));
            if res.len() > pair_count {
                res.pop_last();
            }
        }
    }
