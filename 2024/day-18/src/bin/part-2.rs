...
fn find_path(map: &[Vec<bool>]) -> Option<Node>{
    let width = map[0].len() as i32;
    let height = map.len() as i32;
    let mut queue: BinaryHeap<Reverse<Node>> = BinaryHeap::with_capacity(5000);
    queue.push(Reverse(Node {
        x: 0,
        y: 0,
        count: 0,
    }));
    let mut cache = HashSet::with_capacity(5000);
...
fn part_2(input: &str, width: i32, height: i32, mut iterations: usize) -> String {
    ...
    map[falling[iterations].0 as usize][falling[iterations].1 as usize] = true;

    while find_path(&map).is_some() {
        iterations += 1;
        map[falling[iterations].0 as usize][falling[iterations].1 as usize] = true;
    }

    format!("{},{}", falling[iterations].1, falling[iterations].0).to_string()
}

fn main() {
    let input = include_str!("input.txt");
    let result = part_2(input, 71, 71, 1024);
