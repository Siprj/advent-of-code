...
fn part_2(input: &str) -> String {
    ...
    for n in numbers.iter() {
        ...
        let mut diff2 = 0;
        let mut diff3 = 0;
        let mut changes: HashMap<(i8, i8, i8,i8), u64> = HashMap::new();
        for i in 0..2000u64 {
            ...
            old_digit = digit;
            if i >= 3 {
                changes.entry((diff1, diff2, diff3, diff)).or_insert(digit as u64);
            }
        ...
        for (k, v1) in changes.iter() {
            all_changes.entry(*k).and_modify(|v2| *v2 += *v1).or_insert(*v1);
        }
    }
